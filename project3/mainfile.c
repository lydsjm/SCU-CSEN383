...
char seats_matrix[CONCERT_ROW][CONCERT_COL][5]; 
...
int compareArrivalTime(void *value1, void *value2)
{
...
Queue *generateCustomerQueue(int N)
{
    ...
    sort(customerQueue, compareArrivalTime);
...
int findAvailableSeat(char sellerType)
{
    ...
    else if (sellerType == 'M')
    {
        ...
        while (1)
        {
            ...
            if (mid + jumpRow >= CONCERT_ROW && mid - jumpRow < 0)
                break;
            jumpRow++;
        }
    }
...
void *sell(void *seller)
{
    ...
    while (current_time_slice < sim_duration)
    {
        ...
        if (c != NULL)
        {
            if (randomWaitTime == 0)
            {
                ...
                if (seatIdx == -1)
                {
                    printf("00:%02d     %c%d        Customer Num %c%d%02d has been told concert is sold out.\n",
    ...
    while (c != NULL || sellerQueue->size > 0)
    {
        if (c == NULL)
            c = (Customer *)dequeue(sellerQueue);
        printf("00:%02d     %c%d        Ticket Sale Closed. Customer Num %c%d%02d Leaves\n",
               current_time_slice, sellerType, sellerNum, sellerType, sellerNum, c->customerNum);
        c = NULL;
    }
    pthread_mutex_lock(&threadCreatedMutex);
...
int main(int argc, char **argv)
{
    ...
    printf(" ---------------------------------------------------------\n");
    printf("|%3c | No of Customers | GotSeat  | TurnAway | Throughput|\n", ' ');
    printf(" ---------------------------------------------------------\n");
